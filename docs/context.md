...
### 2026-05-11 — In-app notifications module (group invites)

New `notifications` table + `notification_type` enum (`group_invite`, `invite_accepted`, `invite_rejected`). Notifications are created server-side when: an existing user is invited to a group, an invite is accepted/rejected (inviter gets notified). Pusher `user-{userId}` channel delivers real-time nudges. Counter header has an envelope `NotificationButton` with unread clay dot. `/notifications` page (outside footer nav) shows a scrollable list with accept/reject actions on invite cards, `⋯` expand for toggle-read + delete with confirm, empty-state illustration. When accepting from notifications, `acceptInvitationById` claims the invite by ID (not token), applies snapshot, and redirects to `/`. All existing invite flows (`acceptInvitation`, `rejectInvitation`, `cancelInvitation`) also create/cleanup notifications. Push notifications via Web Push API + VAPID. Settings visual tab has an on/off toggle for push. Run `npm run db:push` after pulling. New files: `src/components/notification-button.tsx`, `src/components/notifications-screen.tsx`, `src/app/notifications/page.tsx`, `src/lib/web-push.ts`, `docs/prompts/notifications-empty.md`.
