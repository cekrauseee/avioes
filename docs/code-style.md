...
- **UI strings are internationalized.** Every visible string flows through `t(locale, key)` from `packages/i18n/src/index.ts`. Brazilian Portuguese is the default; English ships alongside it.
...
- Mutations go through Server Actions in `apps/web/src/actions.ts`. Don't add fetch routes for mutations.
...
The component tree is shallow on purpose. Most screens live in a single `*-screen.tsx` or `*-view.tsx` file under `apps/web/src/components/`. Extract a component when one of these is true:
...
When extracting, keep the new file small and focused: one default export per file, props typed inline with the component, motion/animation logic colocated with the markup. Generic primitives (`expandable-item`, `confirm-row`, `animated-list`, `skeleton`) live in `apps/web/src/components/` next to the consumers — there is no separate `ui/` folder.

## Internationalization

Every visible string goes through `t(locale, key)` or `tf(locale, key, vars)` from `packages/i18n/src/index.ts`. Hardcoded JSX text is a bug.

- **Adding a key**: append to the `pt` object in `packages/i18n/src/index.ts` first (it defines the `TranslationKey` union), then add the same key to `en` with the equivalent string. TypeScript will fail the build if `en` is missing a key. Group keys by feature prefix (`auth.*`, `groups.*`, `settings.*`, `email.*`).
...
- **Date/number formatting**: `Intl.DateTimeFormat(DATE_LOCALE[locale], …)` with the map exported from `packages/i18n`. Don't hardcode `'pt-BR'` or `'en'` at call sites.
- **Email templates**: same `t` / `tf` API, same key file. Subject, preview, body, and plain-text fallback all go through `packages/i18n` — no English-only fallbacks slipping through. Email templates live in `packages/auth/src/email/`.
...
- **Never trust a client `userId` / `groupId`**. Every server action and route resolves the current user via better-auth (`auth.api.getSession({ headers })`) and re-derives the active group through `readGroupMembership(groupId, userId)`. The helpers in `packages/auth/src/guards.ts` (`requireUser`, `requireActiveGroup`, `requireGroupMember`, `requireGroupOwner`) are the canonical entry points; route guards and server actions both go through them.
...
- **Rate-limit at the server**. Anything that emails or invites is rate-limited inside the same DB transaction (`pg_advisory_xact_lock`, partial unique indexes, hourly counters). Rate-limit helpers live in `packages/auth/src/rate-limit.ts`. Client throttles are UX, not security.
...
- For per-member classes, use `MEMBER_COLORS` + `getMemberColor` from `@airplanes/types`.
- Custom theme tokens go in `apps/web/src/app/globals.css` under `@theme inline` and are exposed via CSS variables in `:root` / `[data-theme="dark"]`.
...
- Use `@airplanes/*` for shared package imports. Use the `@/*` path alias for cross-tree imports within an app. Relative imports for sibling files are fine.
...
- Run on Node (no browser) for `apps/web/src/lib/streaks.ts`, offline model tests, and persistence migration helpers.
